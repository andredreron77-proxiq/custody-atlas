...
import { buildDocumentQASystemPrompt, buildExtractedFactsBlock } from "./lib/documentQuestionPrompt";
...
3. If court_address is not printed in the document, return null even if you know the court's real address.`;
  ...
  app.post("/api/ask-document", requireAuth, async (req, res) => {
    try {
      ...
      const extractedFactsBlock = buildExtractedFactsBlock(documentAnalysis.extracted_facts);

      const systemPrompt = buildDocumentQASystemPrompt(userQuestion);
